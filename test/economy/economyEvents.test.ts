...
import { EventManifest, MoneyLedger } from 'types/LifeEvent';
...
describe('economy in the event engine (task 017)', () => {
    test('adjustMoney credits the subject through the ledger', () => {
        ...
        engine.simulateTick(state, ['a'], 0, TPY, { markets: { ledger } });
    ...
    test('the money attribute gates eligibility (only the wealthy qualify)', () => {
        ...
        const result = engine.simulateTick(state, ['rich', 'poor'], 0, TPY, { markets: { ledger } });
